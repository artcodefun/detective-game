...
class _NotebookScreenState extends State<NotebookScreen> {
  ...
  List<(ChronologyEntry, NotebookEntry)> get _taggedNotes {
    final result = <(ChronologyEntry, NotebookEntry)>[];
    for (final chron in _session.chronology) {
  ...
  void _openTagSheet(ChronologyEntry chron, NotebookEntry entry) {
    showModalBottomSheet(
      ...
      builder: (_) => _TagNoteSheet(
        entry: entry,
        onApply: (tags, note) {
          final hasNote = note != null && note.isNotEmpty;
          final updated = _session.updateNotebookEntry(
            chronologyId: chron.id,
            entryId: entry.id,
            userTags: tags,
  ...
  Widget _buildTimeline(ThemeData theme) {
    if (_session.chronology.isEmpty) {
      return Center(
        child: Column(
    ...
    return ListView.builder(
      ...
      itemCount: _session.chronology.length,
      itemBuilder: (_, index) {
        final chron = _session.chronology[index];
  ...
  Widget _buildDetails(ThemeData theme, ColorScheme colorScheme) {
    ...
    return Column(
      children: [
        ...
        ...details.map((detail) => _DetailTile(
          entry: detail,
